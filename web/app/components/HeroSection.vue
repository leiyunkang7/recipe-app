...
<template>
  ...
  <header
    v-if="isMobile"
    class="relative overflow-hidden bg-gradient-to-br from-orange-400 via-orange-500 to-amber-500 md:hidden"
  >
    <!-- 渐变背景 + 动态光效 -->
    <div class="absolute inset-0 overflow-hidden">
      <div class="absolute -top-1/2 -right-1/4 w-96 h-96 rounded-full bg-white/10 blur-3xl animate-pulse"></div>
      <div
  ...
  <header
    v-else
    class="relative overflow-hidden bg-gradient-to-br from-orange-400 via-orange-500 to-amber-500 hidden md:block"
  >
    <!-- 渐变背景 + 动态光效 -->
    <div class="absolute inset-0 overflow-hidden">
      <div class="absolute -top-1/2 -right-1/4 w-[600px] h-[600px] rounded-full bg-white/10 blur-3xl animate-pulse"></div>
      <div
        class="absolute -bottom-1/2 -left-1/4 w-64 h-64 bg-white/10 rounded-full blur-2xl animate-pulse"
        style="animation-delay: 1000ms;"
      ></div>
    ...
    <!-- 内容区 -->
    <div
      class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 transition-all duration-500"
      :class="isEntered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'"
    >
